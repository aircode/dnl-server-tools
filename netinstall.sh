...
dnlstGithubRepo="aircode/dnl-server-tools"
...
if [[ "$steamcmd_user" == "--me" && -z "$userinstall" ]]; then
  ...
  echo "A user-install will create ~/.config/dnlmanager/instances/main.cfg"
  echo "This config file will override /etc/dnlmanager/instances/main.cfg"
...
function doInstallFromCommit(){
  local commit="$1"
  tmpdir="$(mktemp -t -d "dnl-server-tools-XXXXXXXX")"
  ...
  if [ "$result" = 0 ] || [ "$result" = 2 ]; then
    echo "DnL Server Tools successfully installed"
  else
    echo "DnL Server Tools install failed"
  fi
  return $result
}
...
function doInstallFromBranch(){
  channel="$1"
  commit="`curl -s "https://api.github.com/repos/${dnlstGithubRepo}/git/refs/heads/${channel}" | sed -n 's/^ *"sha": "\(.*\)",.*/\1/p'`"
